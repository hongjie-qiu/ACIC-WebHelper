...
async function fetchArticles(keyword) {
  ...
  const response = await fetch("https://cors-anywhere.herokuapp.com/https://serpapi.com/search.json?engine=google_scholar&as_ylo=2022&num=20&start=0&q=\"" + keyword + "\"&api_key=" + apiKey);
...
async function fetchLinks(data) {
  for (let index = 0; index < data.length; index++) {
    ...
    const response = await fetch("https://cors-anywhere.herokuapp.com/" + data[index]['inline_links']['serpapi_cite_link'] + "&api_key=" + apiKey);
    const results = await response.json();
    data[index]['bibtex_link'] = results["links"][0]["link"];
    delete data[index]['inline_links'];

    // extracting bibtex info
    let bibtex = await fetch("https://cors-anywhere.herokuapp.com/" + results["links"][0]["link"]);
